...
const REPORT_CONFIG: Record<ReportPreviewId, { endpoint: string; filename: string }> = {
...
const REPORT_CARDS: ReportCard[] = [
    ...
    },
    {
        id: 'overallocated',
        title: 'Overallocated',
        description: 'Focused heatmap showing only resources with negative bandwidth in any upcoming week.',
        icon: AlertTriangle,
...
export interface ReportsWorkspaceProps {
    title?: string;
    description?: string;
    /** When set, risk intelligence cards only show matching projects (e.g. PM-managed). */
    projectIdsFilter?: Set<string>;
}

export function ReportsWorkspace({
    title = 'Reports',
    description = 'Live master schedules and capacity reports load automatically in each card. Use Refresh to reload allocation data, or download Excel files for offline use.',
    projectIdsFilter,
}: ReportsWorkspaceProps) {
    const [isRefreshing, setIsRefreshing] = useState(false);
    const [downloading, setDownloading] = useState<ReportPreviewId | null>(null);
    ...
    const loadPreviews = useCallback(async (force = false) => {
        ...
        try {
            const response = await api.get<{ status: string; data: AllReportsPreviewResponse }>(
                '/reports/previews',
                { params: { weeks: 12 } }
            );
    ...
    const scopedDeliveryRisks = useMemo(() => {
        if (!projectIdsFilter || projectIdsFilter.size === 0) return deliveryRisks;
        return deliveryRisks.filter((r) => projectIdsFilter.has(r.projectId));
    }, [deliveryRisks, projectIdsFilter]);

    const scopedSkillForecasts = useMemo(() => {
        if (!projectIdsFilter || projectIdsFilter.size === 0) return skillForecasts;
        return skillForecasts.filter((f) => projectIdsFilter.has(f.projectId));
    }, [skillForecasts, projectIdsFilter]);
        ...
        <div className="space-y-8 p-8">
            ...
            {previewError && (
                <div className="flex items-start gap-3 rounded-lg border border-red-200 bg-red-50 p-4 text-red-800">
                    ...
                    <div>
                        <p className="text-sm font-medium">Failed to load report previews</p>
            ...
            <div className="grid grid-cols-1 gap-6">
                {REPORT_CARDS.map((report) => {
                    ...
                    return (
                        <Card key={report.id} className="overflow-hidden border-gray-200 flex flex-col">
            ...
            <section className="space-y-4">
                ...
                <p className="text-sm text-gray-600">
                    Current delivery risk uses Project_Allocation and the weekly planner. Future capability gaps
                    are planning forecasts only.
                    {projectIdsFilter && projectIdsFilter.size > 0 && (
                        <span> Showing risks for your managed projects.</span>
                    )}
                </p>
                <div className="grid lg:grid-cols-2 gap-6">
                    <Card>
                        ...
                        <CardContent>
                            <DeliveryRiskCards risks={scopedDeliveryRisks.slice(0, 4)} loading={riskLoading} />
                        </CardContent>
                    </Card>
                    <Card>
                        <CardHeader>
                        ...
                        <CardContent>
                            <SkillGapForecastCards
                                forecasts={scopedSkillForecasts.slice(0, 4)}
                                loading={riskLoading}
                            />
                        </CardContent>
                    </Card>
